 ...
 export class Scatter extends BaseG2Chart {
  ...
  private originalData: any[] = []
   ...
   init(axis: Array<ChartAxis>, data: Array<ChartData>) {
    ...
    this.originalData = [...data] // deep copy
...
//清除所有图例过滤，显示所有数据
  ...
  showLegendsByString(legendString: string) {
    ...
    if (legendArray.length === 0) {
      ElMessage({
        message: "⚠️ invaild legend ",
        type: 'error',
        showClose: true,
      })
      console.log('invaild legend ')
      this.showAllLegends()
      return
    }
